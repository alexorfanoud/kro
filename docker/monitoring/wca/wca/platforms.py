...
def _parse_dmidecode_output(dmidecode_str):
    ...
    ram_dimm_size = 0
    ram_dimm_speed = 0
    units = {'GB': 1e9, 'MB': 1e6, 'B': 1, 'KB': 1e3}
    ddr_types = ('DDR4', 'DDR3')
        ...
        if memory_device_params.get('size', None) and \
            ...
            if memory_device_params['type'] in ddr_types:
                ...
                ram_dimm_size += int(memory_device_params['size']['value']) * \
                    units[memory_device_params['size']['unit'].upper()]
...
def get_platform_static_information(strict_mode: bool) -> Measurements:
    ...
    if not _platform_static_information_initialized:
        ...
        try:
            ...
            dmidecode_str = re.sub("No Module Installed", "8192 MB",
                        re.sub("Unknown", "DDR3", dmidecode_raw.decode("utf-8")))
